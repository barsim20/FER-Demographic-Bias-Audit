...
**Baseline vs MLP performance.** The logistic regression baseline and the MLP's overall test accuracy are printed in steps 4 and 6 above. Somewhat counter to the expectation that a non-linear model should have the edge, the baseline (about 51%) actually beats the default MLP run at `lr=0.01` (about 47%) - the learning-rate sweep clarifies why: `lr=0.01` isn't actually the best setting once the full dataset gives the model this many gradient steps per epoch, and the `lr=0.001` run pulls ahead of both at roughly 53%. So the honest takeaway is less "the MLP beats the linear model" and more "the MLP can beat the linear model, but only once its learning rate is tuned for the amount of data it's actually training on."

**Subgroup accuracy and significance.** The accuracy tables and bar charts in step 8, together with the chi-square results in step 9, are the core evidence for this audit. All three axes come back significant here: race (p = 0.004), gender (p < 0.0001) and age (p = 0.03), though gender's p-value is by far the smallest of the three. Two patterns stand out. First, race accuracy ranges from about 42% (White) up to about 54% (Indian), roughly a 13-point spread that the significance test says is not explained by chance alone. Second, and more strikingly, female accuracy beats male accuracy in every one of the seven race groups, by as little as 2 points (Southeast Asian) and as much as 18 points (Middle Eastern). That kind of consistency across every single subgroup is what makes the gender result read as a real, reproducible pattern in this pipeline rather than a fluke of one or two rows in the table.
